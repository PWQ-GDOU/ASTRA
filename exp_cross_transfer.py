...
def load_nozzle_source(seq_len=SEQ_LEN, device="cpu"):
    """Load nozzle ablation trajectory as source pre-training data (normalized depth proxy)."""
    nozzle_path = Path("data/raw/nozzle_ablation_full.csv")
    if not nozzle_path.exists():
        print("  Nozzle data not found, skipping", flush=True)
        return None, None, None
    try:
        import chardet
        raw = nozzle_path.read_bytes()
        enc = chardet.detect(raw)["encoding"] or "gbk"
    except Exception:
        enc = "gbk"
    import pandas as pd
    df = pd.read_csv(nozzle_path, encoding=enc)
    # Use depth column → RUL = time to threshold
    depth_cols = [c for c in df.columns if "depth" in c.lower() or "ablat" in c.lower()]
    time_cols  = [c for c in df.columns if "time" in c.lower() or "t_s" in c.lower()]
    if not depth_cols or not time_cols:
        print(f"  Nozzle columns: {list(df.columns)[:10]}", flush=True)
        return None, None, None
    depth = df[depth_cols[0]].values.astype(np.float32)
    t = df[time_cols[0]].values.astype(np.float32)
    # RUL = remaining time to max depth
    max_t = float(t[-1])
    rul = np.maximum(max_t - t, 0.0)
    first_w = seq_len - 1
    max_rul = float(rul[first_w]) if rul[first_w] > 0 else float(rul[0])
    if max_rul <= 0: return None, None, None
    rul_norm = rul / max_rul
    # normalize depth to 0-1
    dep_norm = (depth - depth.min()) / max(depth.max() - depth.min(), 1e-9)
    Xs, ys, cps = [], [], []
    for end in range(seq_len - 1, len(dep_norm)):
        Xs.append(dep_norm[end-seq_len+1:end+1])
        ys.append(rul_norm[end])
        cps.append(t[end] / max_t)
    if not Xs: return None, None, None
    X = torch.as_tensor(np.array(Xs, np.float32)[:,:,None], dtype=torch.float32, device=device)
    y = torch.as_tensor(np.array(ys, np.float32), dtype=torch.float32, device=device)
    cp = torch.as_tensor(np.array(cps, np.float32)[:,None], dtype=torch.float32, device=device)
    print(f"  Nozzle source: {len(X)} windows  RUL_norm {y.min():.3f}-{y.max():.3f}", flush=True)
    return X, y, cp
...
def load_femto_early(series_list, seq_len=SEQ_LEN, feature_group="base",
    ...
    for s in series_list:
        ...
        bearing_data[s.name] = {
            "X_early": rms_norm[:, :, None].astype(np.float32),  # (N_e, L, 1)
            "y_early": rul_norm_e.astype(np.float32),
            "cp_early": cp_e[:, None],
            "X_full":  full.X.astype(np.float32),   # multi-feature for scratch eval
...
def run_cross_transfer_loo(source_X, source_y, source_cp, bearing_data,
    ...
    pretrain_states = []
    for seed in SEEDS:
        m = TransferRULModel(n_in=1, hidden=HIDDEN, dropout=DROPOUT).to(device)
        ...
        for N in N_SHOTS:
            ...
            for trial in range(N_TRIALS):
                ...
                full_X   = torch.as_tensor(
                    bearing_data[holdout]["X_early"][:len(full_rul)],
                    dtype=torch.float32, device=device)

                for seed_i, seed in enumerate(SEEDS):
                    # Transfer
                    mt = TransferRULModel(n_in=1, hidden=HIDDEN, dropout=DROPOUT).to(device)
                    ...
                    # Scratch
                    ms = TransferRULModel(n_in=1, hidden=HIDDEN, dropout=DROPOUT).to(device)
