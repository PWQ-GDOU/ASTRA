"""Reaction wheel v2 controlled seq_len=20 ablation.

Identical to exp_rw_v2_full.py in every respect EXCEPT:
  - SEQ_LEN = 20  (vs 30 in exp_rw_v2_full.py)
  - Candidate names carry 'l20' suffix for traceability
  - Output directory: outputs/reaction_wheel_v2_seq20/

Purpose: isolate the contribution of sequence length from architecture
improvements (deeper models, cycle_pos_norm, larger hidden size).
Comparison table:
  strict_v1     seq_len=20  budget=200  2 models
  strict_v2     seq_len=20  budget=200  2 models
  v2_seq20      seq_len=20  budget=500  5 models  ← this script
  v2_full       seq_len=30  budget=500  5 models  ← exp_rw_v2_full.py
"""
...
SEQ_LEN = 20                    # ← only difference from exp_rw_v2_full.py
...
CANDIDATES = (
    Candidate("deep_ms_full_l20",  "deep_ms",   "full"),
    Candidate("deep_ms_trend_l20", "deep_ms",   "base_trend"),
    Candidate("large_gru_full_l20","large_gru",  "full"),
    Candidate("ms_full_l20",       "ms",         "full"),
    Candidate("gru_full_l20",      "gru",        "full"),
)
...
def run_loo(all_series, candidates, device, epochs, *, seeds, batch_size,
    ...
    for holdout_name in holdout_names:
        ...
        # Ridge baseline (full features, same seq_len=SEQ_LEN=20)
...
def main():
    ...
    parser.add_argument("--output",   default="outputs/reaction_wheel_v2_seq20")
    ...
    checkpoint_path = Path(args.checkpoint) if args.checkpoint else (
        out_dir / "RW_V2_SEQ20_CHECKPOINT.json"
    )
    checkpoint_kwargs = {
    ...
    print(f"RW-v2-SEQ20  device={device}  epochs={args.epochs}  seq_len={SEQ_LEN}",
          flush=True)
    ...
    print(f"\n{'='*60}", flush=True)
    print("RW-v2-SEQ20 — LOO macro results", flush=True)
    ...
    report = {
        "schema":         "reaction_wheel_v2_seq20",
        "seq_len":        SEQ_LEN,
        "epochs":         args.epochs,
        "batch_size":     args.batch_size,
        "patience":       args.patience,
    ...
    (out_dir / "RW_V2_SEQ20_REPORT.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2, allow_nan=False),
        encoding="utf-8")

    with open(out_dir / "rw_v2_seq20_macro.csv", "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["candidate", "macro_nrmse"])
        for cn, v in sorted(macro.items(), key=lambda x: x[1]):
            w.writerow([cn, f"{v:.4f}"])

    print(f"Report -> {out_dir}/RW_V2_SEQ20_REPORT.json", flush=True)
    write_heartbeat(
        heartbeat_path,
        phase="completed",
        report=str(out_dir / "RW_V2_SEQ20_REPORT.json"),
    )
