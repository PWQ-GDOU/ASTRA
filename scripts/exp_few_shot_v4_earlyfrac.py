"""Few-shot v4: early_frac sensitivity sweep.

Phase1 v3 fixed EARLY_FRAC=0.20 (the few-shot pool is drawn only from the first
20% of each training trajectory's life). This script re-runs the identical v3
transfer/scratch/ridge comparison at multiple EARLY_FRAC values to check whether
the headline conclusion (transfer beats scratch and ridge at low N) is an
artifact of that specific choice or holds across a range.

Only EARLY_FRAC varies across sweep points. Everything else (splits, early
stopping, seeds, trials, model, Ridge baseline, NASA pretraining) is identical
to v3 and shared across sweep points where possible (pretraining is source-side
and does not depend on target early_frac, so it is computed once and reused).
"""
...
# ── constants (identical to v3 except EARLY_FRAC_VALUES) ─────────────────
EARLY_FRAC_VALUES = (0.10, 0.20, 0.30, 0.40)
EPOCHS_PRE   = 400
EPOCHS_FT    = 300
EPOCHS_SCR   = 500
SEQ_LEN      = 16
...
# ── data (identical to v3, early_frac is now a parameter) ────────────────
...
def soh_from_geo(df_traj, seq_len, early_frac, eol_soh=0.80):
...
def run(device, out_dir, quick=False):
    ...
    # Test set is built from FULL-trajectory windows (Xf) which do NOT depend on
    # early_frac at all -- so it is identical across every early_frac value tested.
    # This keeps the sensitivity sweep a true single-variable ablation.
    geo_test_Xf, geo_test_yf, geo_test_maxruls = [], [], []
    for tid in test_ids:
        sub = geo_df[geo_df["trajectory_id"]==tid]
        d = soh_from_geo(sub, SEQ_LEN, early_frac=0.20)  # early_frac irrelevant for Xf/yf
        if d is not None:
    ...
    print(f"\nPre-training on NASA ({len(NASA_AUX)} cells, {epochs_pre} epochs)"
          f" -- shared across all early_frac values...", flush=True)
    pretrain_states = []
    for seed in SEEDS:
    ...
    n_shots  = N_SHOTS if not quick else (1, 5, 20)
    all_frac_results = {}

    for early_frac in EARLY_FRAC_VALUES:
        print(f"\n{'#'*90}", flush=True)
        print(f"# early_frac = {early_frac}", flush=True)
        print(f"{'#'*90}", flush=True)

        geo_train_data = {}
        for tid in train_ids:
            sub = geo_df[geo_df["trajectory_id"]==tid]
            d = soh_from_geo(sub, SEQ_LEN, early_frac=early_frac)
            if d is not None:
        ...
        for N in n_shots:
            ...
            for trial in range(n_trials):
                ...
                for seed_idx, seed in enumerate(SEEDS):
                    ...
                    mt, _ = train_model_es(mt, Xtr, ytr, Xval, yval, device, seed, epochs_ft,
                                            freeze_encoder=True)
                    ...
                    ms, _ = train_model_es(ms, Xtr, ytr, Xval, yval, device, seed, epochs_scr,
            ...
            results[N] = {
                "n_shot": N, "n_train_traj_avail": len(avail),
            ...
            print(f"  early_frac={early_frac}  N={N:3d}: Transfer={t_mean:.2f}±{t_std:.1f}"
                  f"  Scratch={s_mean:.2f}±{s_std:.1f}  Ridge={r_mean:.2f}±{r_std:.1f}"
                  f"  GainVsScratch={gain_vs_scratch:+.1f}%  GainVsRidge={gain_vs_ridge:+.1f}%", flush=True)

        all_frac_results[str(early_frac)] = results

    print(f"\n{'='*100}", flush=True)
    print("EARLY_FRAC SENSITIVITY SUMMARY (Transfer gain_vs_scratch_pct by N)", flush=True)
    print(f"{'='*100}", flush=True)
    header = f"{'early_frac':>12}" + "".join(f"{'N='+str(N):>12}" for N in n_shots)
    print(header, flush=True)
    for ef, res in all_frac_results.items():
        row = f"{ef:>12}"
        for N in n_shots:
            if N in res:
                row += f"{res[N]['gain_vs_scratch_pct']:>+11.1f}%"
            else:
                row += f"{'n/a':>12}"
    ...
    report = {
        "schema": "few_shot_v4_earlyfrac_sensitivity",
        "purpose": ("Sweeps EARLY_FRAC (fraction of each source trajectory used as the "
                    "few-shot pool) to test whether v3's transfer-vs-scratch-vs-ridge "
                    "conclusions are stable, or an artifact of the EARLY_FRAC=0.20 choice. "
                    "Test set is built from full-trajectory windows and does not depend on "
                    "early_frac, so it is identical across all rows -- a true single-variable ablation."),
        "early_frac_values": list(EARLY_FRAC_VALUES),
        ...
        "results_by_early_frac": all_frac_results,
    }
    (out_dir / "FEW_SHOT_V4_EARLYFRAC_REPORT.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    import csv as _csv
    with open(out_dir / "few_shot_v4_earlyfrac_curve.csv", "w", newline="") as f:
        w = _csv.writer(f)
        w.writerow(["early_frac","n_shot","method","rmse","std"])
        for ef, res in all_frac_results.items():
            for N, r in res.items():
                w.writerow([ef, N, "transfer", r["transfer_mean_rmse"], r["transfer_std_rmse"]])
                w.writerow([ef, N, "scratch",  r["scratch_mean_rmse"],  r["scratch_std_rmse"]])
                w.writerow([ef, N, "ridge",    r["ridge_mean_rmse"],    r["ridge_std_rmse"]])
    print(f"Report → {out_dir}/FEW_SHOT_V4_EARLYFRAC_REPORT.json", flush=True)
    return all_frac_results


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--output", default="outputs/few_shot_v4_earlyfrac")
    parser.add_argument("--device", default="cuda:1")
    parser.add_argument("--quick",  action="store_true")
    args = parser.parse_args()
    device = args.device if torch.cuda.is_available() else "cpu"
    print(f"Few-shot v4 early_frac-sensitivity  device={device}  quick={args.quick}"
          f"  values={EARLY_FRAC_VALUES}", flush=True)
    t0 = time.time()
